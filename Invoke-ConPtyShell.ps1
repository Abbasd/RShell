function Invoke-ConPtyShell {
    ...
    $width = $cols
    $height = $rows
    $psWindow = [System.Diagnostics.Process]::GetCurrentProcess() | Get-Process | Select-Object -ExpandProperty MainWindowHandle
    $consoleWindow = [User32]::FindWindow("ConsoleWindowClass", $null)
    $consoleInfo = New-Object User32.CONSOLE_SCREEN_BUFFER_INFOEX
    $consoleInfo.cbSize = [Runtime.InteropServices.Marshal]::SizeOf($consoleInfo)
    $consoleInfo.dwSize.X = $width
    $consoleInfo.dwSize.Y = $height
    $consoleInfo.srWindow.Right = $width - 1
    $consoleInfo.srWindow.Bottom = $height - 1
    [User32]::SetConsoleScreenBufferInfoEx($consoleWindow, $consoleInfo)
    [User32]::MoveWindow($psWindow, 0, 0, $width * 8, $height * 8, $true)
